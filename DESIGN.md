...
| `?date=X&meal=Y&dish=Z&ingredients=A,B,C&memo=M` | 新規記録（料理名＋食材＋メモプリセット） | AI提案経由 |
| `?date=X&meal=Y&edit=1` | **編集モード** | 履歴「編集する」経由 |

### プリセットの優先順位（log.html）
- 料理名：AIパラメータ → 既存ログ（既存があれば上書き）
- 食材：既存ログに食材がある場合は既存優先。ない場合はAIパラメータを使用
- メモ：既存ログにメモがある場合は既存優先。ない場合はAIパラメータを使用

### 食材入力の仕様（v0.9.1更新）
- ラベル：「使用食材」（旧：「使用食材（スペースで区切って入力）」）
- プレースホルダー：「食材名を入力してEnter（複数可）」
- Enterで確定・タグとして表示
- 入力値を全角スペース・半角スペース・読点・カンマで自動分割して複数タグ登録
- タグの×ボタンで個別削除
- タグをタップしても何も起きない（削除は×ボタンのみ）
...
8. log.htmlで料理名・食材タグ・メモ欄にプリセット表示（既存ログがある場合は既存優先）
...
- **思想**: 常備調味料は登録不要。賞味期限が近いもの・使い切りたいものだけ登録する運用を想定
- **上限**: 30個。残り5個以下でガイドメッセージ表示。上限到達時は追加不可＋メッセージ表示
...
- 食材入力?ツールチップ追加・星評価を大型化・「家族の評価」→「また食べたい度」に変更 ✅ v0.9.3
- LP（index.html）新規作成・利用規約・プライバシーポリシー・お問い合わせページ追加 ✅ v1.0.0
...
### v1.0.0 公開対応（完了）
- LP（index.html）新規作成：ヒーロー・機能紹介・AIモック・使い方3ステップ・CTA・運営者情報・フッター
- 利用規約（terms.html）新規作成
- プライバシーポリシー（privacy.html）新規作成
- お問い合わせ（contact.html）新規作成（Formspree: xpqbkdea、Taskraと同一エンドポイント）

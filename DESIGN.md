...
（説明）
```
さらに冷蔵庫食材・今週の記録・好評メニューをsystemPromptに動的に追記。

### parseDishBlocks() の動作（suggest.html内）
- `①②③` または `1.` で始まる行 → 料理名として抽出（2〜25文字）
- `食材：` または `材料：` で始まる行 → `・` 区切りで食材リストに変換
- `・` `-` で始まる短い行（15文字以内）→ 箇条書き食材として補完抽出
- 抽出した `{ name, ingredients[] }` をボタン生成に渡す

...
|---|---|---|---|
| 1 | ホーム | home.html | |
| 2 | 履歴 | history.html | 「ログ」→「履歴」に戻した（ヘッダー「献立履歴」と統一） |
| 3 | ＋ | log.html | 中央・テラコッタ丸ボタン（56px・Flowra準拠）・ラベルなし |
| 4 | AI提案 | suggest.html | |
| 5 | 設定 | settings.html | |

- FABフローティングボタン廃止（冗長だったため）
- 「記録」タブ廃止 → ＋ボタンに一本化
- 「履歴」→「ログ」→「履歴」に戻した（ヘッダー「献立履歴」と統一するため）
- ＋ボタンをFlowra準拠デザインに（56px・margin-top:-20px・影付き）

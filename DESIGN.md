...
### レイアウト（v1.10.0）
- モバイル：上部にタブ（👥ユーザー管理 / 🤖AIモデル）でセクション切替
- iPad/PC（≥769px）：左サイドバーナビ（`.admin-sidebar`）でセクション切替。`.admin-wrap`をCSS Gridに切替（220px固定サイドバー＋残り幅メイン）
- `showSection('users' | 'aimodel')` でセクションの表示切替（`.hidden`クラスのトグル）
...
### 機能（収益構造セクション・v1.11.0新設）
- フリープラン・プレミアムプランそれぞれの売価・原価・粗利・利益率を表で比較
- 価格はLP（index.html `#pricing`）と同じ値を使用（**手動同期**：プラン価格や上限を変更する際は、index.html・settings.html・tavera-suggest（Edge Function）・admin.htmlの4箇所を必ず揃えて更新すること。ビルド時に自動参照する仕組みは無い）

| 項目 | 無料プラン | プレミアムプラン |
|------|----------|----------------|
| 売価（月額） | ¥0 | ¥480 |
| AI相談 上限回数 | 10回/月（1日3回） | 500回/月（1日50回） |
| AI相談 原価（上限利用時・¥160/$換算） | ¥4 | ¥200 |
| 献立取り込み 上限回数 | **上限なし（無制限）** | **上限なし（無制限）** |
| 食材写真取り込み 上限回数 | **上限なし（無制限）** | **上限なし（無制限）** |
| 原価合計（AI相談のみ・確定分） | ¥4 | ¥200 |
| 粗利（確定分） | −¥4 | ¥280 |
| 利益率（確定分） | ―（売価¥0） | 58.3% |

- **重要な発見**：`tavera-kyushoku`（献立取り込み）と`tavera-fridge-scan`（食材写真取り込み）には、フリー/プレミアムともに利用回数の上限チェックが**コード上に一切存在しない**（`tavera-suggest`のAI相談だけがmenu_ai_usageで月次・日次の上限管理をしている）。そのため上表の原価・粗利・利益率は「AI相談のみを反映した確定分」であり、残り2機能を大量利用された場合はこの数字を超えて原価が増加するリスクがある。
- 対応候補：`tavera-kyushoku`・`tavera-fridge-scan`にも`menu_ai_usage`と同様の月次/日次カウント・上限チェックを追加する（要望があれば実装可能）

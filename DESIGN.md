...
### 2026-08-30: 紹介特典を「双方プレミアム1ヶ月無料」に変更

- **変更前**: 双方に AI提案+10回/月（referral_bonus jsonb）。紹介成立0件で、特典が伝わらないことが原因の一つと判断
- **変更後**: 双方に **プレミアム31日**（紹介者は最大5人分まで）。`tavera-referral` v3 でサーバー側のみで付与
- 付与ロジック `grantPremium()`: **Stripe課金中（stripe_subscription_id あり）の人はスキップ**（Stripe側を正とし二重管理を避ける）。既存プレミアム（手動/紹介分）は期限に31日積み増し
- 不正対策は従来どおり: 登録30日以内のみ・1回きり（referee_id UNIQUE）・自分/同一世帯は不可
- フロント（settings.html）: 説明文・招待メッセージ（LINE向け）を新特典に更新
- **セキュリティ**: plan 変更は service role の Edge Function 内のみ。RLS・認証・Stripe フローに変更なし。旧 referral_bonus カラムは残置（既存の+10回ボーナスは有効なまま）
- 同日: ブログ枠（blog/）新設・記事2本公開・sitemap.xml 追加・GSC プロパティ登録（googlea329e4ef453dbb49.html は削除しない）
...
### アカウントメニューに「アカウント切り替え」追加・「サインアウト」→「ログアウト」に統一（v1.26.2）

- home.htmlのアバターメニューに「🔄 アカウント切り替え」を追加（ログアウト→`index.html?lp=1`でログイン済みリダイレクトを回避しLPのログイン導線へ直行）。「サインアウト」表記を設定画面と同じ「ログアウト」に統一。デモ用アカウントへの切り替え用途

### アカウントメニューから「キャンセル」を削除（v1.26.3）

- メニュー外タップで閉じる実装が既にあるためキャンセル項目は冗長と判断し削除。「メール／🔄アカウント切り替え／🚪ログアウト」の3行構成に簡素化（Taskraのメニューを参考）

### アカウント切り替えを本来の挙動に修正（v1.26.4）

...
| `?date=X&meal=Y&dish=Z&ingredients=A,B,C&memo=M` | 新規記録（料理名＋食材＋メモプリセット） | AI提案経由（v0.9.0追加） |
| `?date=X&meal=Y&edit=1` | **編集モード** | 履歴「編集する」経由 |

### プリセットの優先順位（log.html）
- 料理名：AIパラメータ → 既存ログ（既存ログがあれば上書き）
...
{
  name: "トマトソースパスタ",           // 料理名
  ingredients: ["パスタ", "トマト缶"],  // 食材リスト
  description: "あっさりとしたトマト..."  // 説明文（1文目）
}
```

抽出ロジック：
- `①②③` または `1.` で始まる行 → 料理名（2〜25文字）
- `食材：` または `材料：` で始まる行 → `・` 区切りで食材リスト
- `・` `-` で始まる短い行（15文字以内）→ 箇条書き食材
- 句読点を含む10文字以上の行 → 説明文（1文のみ取得）
